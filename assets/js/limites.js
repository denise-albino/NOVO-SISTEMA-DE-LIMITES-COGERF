...
// --- global idempotent delegation (garante handlers para elementos dinâmicos) --- if (!window._limitesDelegationInstalled) { window._limitesDelegationInstalled = true; document.addEventListener('click', function(e){ const btn = e.target.closest && e.target.closest('[data-action]'); if(!btn) return; const action = btn.getAttribute('data-action'); if(!action) return; try{ if(action==='remove-block'){ const block=btn.getAttribute('data-block'); if(block){ const el=document.getElementById(block); if(el) el.remove(); if(typeof saveDraft==='function') saveDraft(); if(typeof buildResumo==='function') buildResumo(); if(typeof updateSalvarState==='function') updateSalvarState(); } return; } if(action==='add-remaj-leg'){ const remaj=btn.getAttribute('data-remaj'), role=btn.getAttribute('data-role'); if(remaj && role){ const containerId=(role==='orig'?'orig-':'dest-')+remaj; if(typeof addRemajLeg==='function') addRemajLeg(containerId, role, parseInt(remaj,10)); } return; } if(action==='distribuir'){ const uid = parseInt(btn.getAttribute('data-uid'),10); if(!isNaN(uid) && typeof distribuirIgual==='function') distribuirIgual(uid); return; } if(action==='remove-gid'){ const uid = parseInt(btn.getAttribute('data-uid'),10); if(!isNaN(uid) && typeof removeGid==='function') removeGid(uid); return; } if(action==='add-supl' || action==='addSupl'){ if(typeof addSupl==='function') addSupl(); return; } if(action==='add-remaj' || action==='addRemaj'){ if(typeof addRemaj==='function') addRemaj(); return; } if(window._limitesActionHandlers && typeof window._limitesActionHandlers[action]==='function'){ window._limitesActionHandlersaction; return; } }catch(err){ console.error('delegation handler error',err); } }, true); // usando capture:true para interceptar antes de event.stopPropagation console.debug('limites: global delegation installed'); }
